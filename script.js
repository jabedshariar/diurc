...
function renderTeamMembers() {
    ...
    MOCK_TEAM_MEMBERS.forEach(member => {
        ...
        card.innerHTML = `
            <img src="${member.img}" onerror="this.onerror=null;this.src='https://placehold.co/90x90/1E415A/00D4FF?text=User'" alt="${member.name}">
            <h4>${member.name}</h4>
...
function initSlideshow() {
    ...
    const slides = images.map((url, index) => {
        let content = (index === 0) ? "Building the Future of Automation" : (index === 1) ? "Join Our Next Project Cycle" : "DIU Robotics Club: Innovation Driven";
